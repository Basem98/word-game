    ...
    .then((res) => {
      if (res && res.isAuthorized) {
        ...
        if (currentUser.gameHistory.length > 0) {
          currentUser.gameHistory.forEach((wordObj) => {
            const wordsTableBody = document.getElementsByTagName('tbody');
            ...
            wordsHistoryTable.style.visibility = 'visible';
            wordsHistoryTable.style.opacity = 1;
          });
